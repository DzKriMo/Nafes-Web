
const chatdiv = document.getElementById("chatt-container")
const viddiv = document.getElementById("videoChat")
const leftArrow = document.getElementById("left");
const rightArrow = document.getElementById("right")

/*
function left(){
  const filename = leftArrow.src.substring(leftArrow.src.lastIndexOf('/') + 1);
  if (filename === "left.png") {
    window.open("https://nafes-59aaa.web.app/", '_blank');
  }
  leftArrow.src = "/img/leftt.png"
  rightArrow.src = "/img/rightt.png"
}
function right() {
  chatdiv.style.display="flex"
  viddiv.style.display="none"
  leftArrow.src = "/img/left.png";
  rightArrow.src= "/img/right.png";
  
}

*/

function videoChat(){
  window.open("https://nafas-therapy.web.app/", '_blank');
}























































































































































































                                                                                                                                    const firebaseConfig = {
                                                                                                                                    apiKey: "AIzaSyDmzv4pNcArlPiAlmVJoAzPaGC9axknz_A",
                                                                                                                                    authDomain: "nafes-59aaa.firebaseapp.com",
                                                                                                                                    databaseURL: "https://nafes-59aaa-default-rtdb.europe-west1.firebasedatabase.app",
                                                                                                                                    projectId: "nafes-59aaa",
                                                                                                                                    storageBucket: "nafes-59aaa.appspot.com",
                                                                                                                                     messagingSenderId: "342707494946",
                                                                                                                                     appId: "1:342707494946:web:ee76897b8c98d7c5f2aa7b",
                                                                                                                                     measurementId: "G-FN2HQK4Q6Q"
                                                                                                                                      };


